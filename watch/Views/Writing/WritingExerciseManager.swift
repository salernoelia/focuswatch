...
class WritingExerciseManager: NSObject, ObservableObject {
    ...
    private func handleExerciseTimerUpdate(_ currentTime: Int) {
        DispatchQueue.main.async { [weak self] in
            ...
            if currentTime <= 0 {
                ...
                if !self.pomodoro,
                    Double(self.totalWorkTime()) >= Double(self.currentSetting.learn) * 60
                {
                    self.stopExercise {}
                } else {
        ...
        writingManager.startWritingManager(isPomodoro: self.pomodoro) { [weak self] startDate in
            ...
            DispatchQueue.main.async {
                if self.recordStartDate == nil {
                    ...
                    print(
                        "Stored recording start date: \(String(describing: self.recordStartDate))")
    ...
    private func monitorExercise() {
        ...
        if (status == 0 || status == 3) && positive_feedback
            && currentTime % posFeedbackInterval == 0
            && checkPosFeedback()
        {
            ...
            if exerciseState != .disrupted {
                ...
                if !positive_feedback && (hapticManager.isHapticFeedbackActive == false) {
                    hapticManager.startHapticFeedback(
                        interval: TimeInterval(currentSetting.negFBInterval))
    ...
    private func createAndUploadSessionJSON() {
        ...
        let projectedEndDate = recordStartDate.addingTimeInterval(
            TimeInterval(projectEndDateInSeconds))
            ...
            dataStorageManager.createSessionJSON(data: self.sessionData, filename: filename) { _ in
                do {
                    let jsonData = try JSONSerialization.data(
                        withJSONObject: self.sessionData, options: [])
...
extension WritingExerciseManager: WKExtendedRuntimeSessionDelegate {
    ...
    func extendedRuntimeSession(
        ...
        case .sessionInProgress:
            logToFile(
                "Session invalidated with reason 'sessionInProgress'. This should not occur here.")
        case .none:
...
extension WritingExerciseManager {
    ...
    func checkPosFeedback() -> Bool {
        ...
        for entry in stateHistory.reversed() {
            if currentTime.timeIntervalSince(entry.timestamp) <= timeInterval,
                entry.state == .disrupted
            {

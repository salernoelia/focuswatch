...
struct ChecklistType: Identifiable, Codable {
    var id = UUID()
    var name: String
    var displayName: String
    var items: [EditableChecklistItem]
    var colorName: String
    ...
    init(name: String, displayName: String, items: [EditableChecklistItem], color: Color) {
        self.name = name
...
struct ChecklistConfiguration: Codable {
    ...
    var bastelItems: [EditableChecklistItem] {
        return checklistTypes.first { $0.name == "basteln" }?.items ?? []
    }
    
    var rezeptItems: [EditableChecklistItem] {
        return checklistTypes.first { $0.name == "rezept" }?.items ?? []
    }
}

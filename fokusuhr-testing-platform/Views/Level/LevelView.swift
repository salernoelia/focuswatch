        ...
        Section {
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack {
                        Text("\(viewModel.levelData.currentXP) Points")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        Spacer()
                        Text("\(viewModel.xpNeededForNext) Points")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                ...
                Text("Total: \(viewModel.levelData.totalXP) Points")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            .padding(.vertical, 4)
...
class LevelViewModel: ObservableObject {
    ...
    private let levelService = LevelSyncService.shared
    private var cancellables = Set<AnyCancellable>()

    init() {
        self.levelData = levelService.loadLevelData()
        setupObservers()
    }
        ...
        NotificationCenter.default.publisher(for: NSNotification.Name("LevelDataUpdated"))
            .sink { [weak self] notification in
                if let updatedData = notification.userInfo?["levelData"] as? LevelData {
                    ...
                    self?.levelData = self?.levelService.loadLevelData() ?? LevelData.default
                    self?.objectWillChange.send()
                }
            }
    ...
    func refresh() async {
        levelData = levelService.loadLevelData()
        levelService.sync()
        objectWillChange.send()
    }
}

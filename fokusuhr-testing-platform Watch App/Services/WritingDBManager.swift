...
class DataStorageManager: UploadService {
    ...
    func createSessionJSON(data: [String: Any], filename: String, completion: @escaping (Result<Void, Error>) -> Void) {
    ...
    ///   - completion: A result handler indicating success or failure.
    func updateSessionJSON(data: [String: Any], filename: String, completion: @escaping (Result<Void, Error>) -> Void) {
        DispatchQueue.global(qos: .background).async {
            do {
                let jsonData = try JSONSerialization.data(withJSONObject: data, options: [.prettyPrinted])
                let fileURL = self.getDocumentsDirectory().appendingPathComponent(filename)
    ...
    func fetchAndStoreAccelerometerData(sessionFilename: String, completion: @escaping (Result<URL, Error>) -> Void) {
    ...
    internal func uploadBinaryData(_ binaryData: Data, sessionFilename: String, completion: @escaping (Result<URL, Error>) -> Void) {
        let filename = (sessionFilename as NSString).deletingPathExtension + ".bin"
        uploadService.uploadDataToGoogleDrive(binaryData: binaryData, filename: filename, folderid: folderIDBin, completion: completion)
    }
    ...
    internal func uploadDataToGoogleDrive(binaryData: Data, filename: String, folderid: String, completion: @escaping (Result<URL, Error>) -> Void) {
    ...
    func uploadToGoogleDrive(data: Data, filename: String, folderID: String, completion: @escaping (Result<Void, Error>) -> Void) {
        UserConfigs.getAccessToken { result in
            ...
            case .success(let accessToken):
                ...
                guard let uploadURL = URL(string: "https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart") else {
                    completion(.failure(NSError(domain: "GoogleDriveUploadError", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid upload URL"])))
                    return
                }
                ...
                let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
                    ...
                    if let httpResponse = response as? HTTPURLResponse, (200...299).contains(httpResponse.statusCode) {
                        ...
                        let error = NSError(domain: "GoogleDriveUploadError", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to upload data to Google Drive"])
                        completion(.failure(error))
                    }
                }
                task.resume()
    ...
    func uploadDataToWebhook(jsonData: Data, completion: @escaping (Result<Void, Error>) -> Void) {
        guard let uploadURL = URL(string: "https://europe-west6-schlautech001.cloudfunctions.net/focuswatch2doc") else {
            completion(.failure(NSError(domain: "WebhookUploadError", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid webhook URL"])))
            return
        }
        ...
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if let httpResponse = response as? HTTPURLResponse, (200...299).contains(httpResponse.statusCode) {
                ...
                let error = NSError(domain: "WebhookUploadError", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to upload JSON payload to webhook"])
                completion(.failure(error))
            }
        }
        task.resume()
    ...
    func uploadLive(stateEntries: [ExerciseManager.StateHistoryEntry], deviceID: String, dateString: String, completion: @escaping (Result<Void, Error>) -> Void) {
    ...
    func tryUploads(completion: @escaping (Result<[URL], Error>) -> Void) {
        ...
        for upload in failedUploads {
            ...
            do {
                ...
                case "bin":
                    self.uploadDataToGoogleDrive(binaryData: data, filename: filename, folderid: self.folderIDBin) { result in
                        syncQueue.async {
                            switch result {
                            case .success(let url):
                ...
                case "json":
                    self.uploadDataToGoogleDrive(binaryData: data, filename: filename, folderid: self.folderIDJSON) { result in
                        syncQueue.async {
                            switch result {
                            case .success(let url):
                                retryUploadedURLs.append(url)
                                ...
                                self.removeFailedUpload(fileURL: fileURL)
                            case .failure(let error):
                                print("Failed to re-upload .json file to Google Drive: \(filename), error: \(error)")
                                if retryError == nil { retryError = error }
                            }
                            dispatchGroup.leave()
                        }
    ...
    func addToUploads(filename: String) {
        ...
        guard fileManager.fileExists(atPath: fileURL.path) else {
            print("File \(filename) does not exist at path: \(fileURL.path). Cannot add to failed uploads.")
            return
        }
    ...
    private func completeOnMainQueue<T>(_ completion: @escaping (Result<T, Error>) -> Void, with result: Result<T, Error>) {
        DispatchQueue.main.async {
            completion(result)
        }
    }
    ...
    private func error(with message: String) -> NSError {
        return NSError(domain: "DataStorageManagerError", code: 1, userInfo: [NSLocalizedDescriptionKey: message])
    }
}

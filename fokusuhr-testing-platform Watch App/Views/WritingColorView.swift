...
struct ColorView: View {
    ...
    var body: some View {
        Group {
            ...
            if exerciseManager.showRunView && !UserConfigs.shared.configs.feedbackEnabled {
                Color.black.edgesIgnoringSafeArea(.all)
            }
            // If the session is running and feedback is enabled, show the main run view.
            else if exerciseManager.showRunView {
                ColorRunView()
            } else {
...
struct ColorRunView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                if exerciseManager.exerciseState != .ended {
                    Color.black.edgesIgnoringSafeArea(.all)
                    ...
                    Circle()
                        .fill(
                            LinearGradient(
                                gradient: Gradient(colors: [exerciseManager.exerciseState.color, exerciseManager.exerciseState.color.opacity(0.7)]),
                                startPoint: .top,
                                endPoint: .bottom
                            )
                        )
    ...
    private func getFillPercentage() -> CGFloat {
        ...
        if exerciseManager.exerciseState == .pausing {
            // During pause, the fill level decreases as the pause timer counts down.
            let pauseTimePercentage = Double(exerciseManager.currentPauseTime) / Double(currentSetting.pause * 60)
            percentage = 1 - pauseTimePercentage
        } else if exerciseManager.pomodoro {
            // In Pomodoro mode, the fill level increases as the work timer counts down.
            let timePercentage = Double(exerciseManager.currentTime) / Double(currentSetting.learn * 60)
            percentage = 1 - timePercentage
        } else {
            // In non-Pomodoro (continuous) mode, the fill level increases based on total accumulated work time.
            let timeWorked = Double(exerciseManager.totalWorkTime())
    ...
    private func getTopPadding(for diameter: CGFloat) -> CGFloat {
        ...
        if exerciseManager.exerciseState == .pausing {
            ...
            if exerciseManager.pomodoro {
                timePercentage = Double(exerciseManager.currentTime) / Double(currentSetting.learn*60)
                percentage = timePercentage
            }
            else {
                let timeWorked = exerciseManager.totalWorkTime()
